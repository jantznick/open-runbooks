...
**Supplementary clarifications and examples** (non-normative; orgs replace with internal links and standards):

- **Why both language-specific and general material:** Many defects are **idiom- and stack-specific** (ORM/query APIs, templating, serialization, async patterns, framework defaults). **Language/framework cheat sheets** address that. **Topic** cheat sheets (e.g. injection, XSS, authentication, secrets storage) and **[OWASP ASVS](https://owasp.org/www-project-application-security-verification-standard/)** are **technology-neutral** verification targets—useful as a contract with AppSec and for test plans regardless of language.
- **Action 1 — example starting points:** **[OWASP Cheat Sheet Series](https://cheatsheetseries.owasp.org/)** (index lists per-language and per-topic sheets). **[OWASP Top 10](https://owasp.org/www-project-top-ten/)** for shared vocabulary. **[ASVS](https://owasp.org/www-project-application-security-verification-standard/)** when you need level-based requirements. Prefer **internal** secure-coding playbooks where they exist (they override public guides for your org).
- **Action 2 — what “hygiene” means here:** This is **developer and repo practice** that makes **D2** and **D3** meaningful backups, not the only control: lockfiles and intentional dependency bumps; secrets only via managers/CI—not in git; short-lived credentials; reviewing **C2** checklist “yes” paths when deps or auth/config change. CI proves scans ran; this action is the **day-to-day discipline** those scans support.
- **Action 3 — where training lives:** This control is the **baseline policy requirement** (who must learn what, by when). **How** you deliver it (LMS, bootcamp, champions, links from findings) is program-specific. The full baseline’s **Evidence** for this control is **completion or acknowledgment where tracked**—see [`appsec-policy-baseline.md`](appsec-policy-baseline.md) (Secure coding standards row). Broader **training paths, champions, and SAMM “Education and Guidance”** are described at program level in [`../program/06-improve-and-govern.md`](../program/06-improve-and-govern.md#adoption-paths) and [`../program/samm-coverage-checklist.md`](../program/samm-coverage-checklist.md); those are complementary, not a substitute for meeting this control’s evidence.
